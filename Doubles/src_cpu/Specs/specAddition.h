...
template<>
inline void baileyAdd_fast<4,2,5>(double const *x, double const *y, double *z){
	double e, f[6];
	f[4] = 0.0;
	f[5] = 0.0;
	f[3] = x[3];
...
template<>
inline void baileyAdd_fast<6,5,4>(double const *x, double const *y, double *z){
	double e, f[5];
	f[4] = FPadd_rn(x[4], y[4]);
	f[3] = two_sum(x[3], y[3], e);
	f[4] = FPadd_rn(f[4], e);
	f[2] = two_sum(x[2], y[2], e);
	f[3] = two_sum(f[3], e, e);
